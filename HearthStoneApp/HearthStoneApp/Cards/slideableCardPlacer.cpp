...
SlideableCardPlacer::SlideableCardPlacer(sf::Vector2f _restingPosition, sf::Vector2f _size, float _silideAmount,
	...
	slide = _silideAmount;
	singleMove = move == MoveType::VERTICAL ? sf::Vector2f(0, 1) : sf::Vector2f(1, 0);
...
SlideableCardPlacer::SlideableCardPlacer(sf::Vector2f _restingPosition, sf::Vector2f _size, float _silideAmount,
	...
	: SlideableCardPlacer(_restingPosition, _size, _silideAmount, _type, _gameHandler) {
...
void SlideableCardPlacer::update() {
	...
	if (move == MoveType::VERTICAL) {
		if (CardPlacer::isCursorHoverdOver()) {
			if (body.getPosition().y > restingPosition.y + slide) CardPlacer::move(singleMove);
		}
		else {
			if (body.getPosition().y < restingPosition.y) CardPlacer::move(-singleMove);
		}
	}
}